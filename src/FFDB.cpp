 ...
 *   initial import
 *
 *   Revision 1.2  2008/08/04 16:34:08  chen
...
namespace FFDB {
   ...
   * Several used macros
   */
#define __FFDB_BSWAP_16(x) \
  (( ( (x) >> 8) & 0xffu) | ( ( (x) & 0xffu) << 8))

#define __FFDB_BSWAP_32(x) \
  (( ( (x) & 0xff000000u) >> 24) | ( ( (x) & 0x00ff0000u) >> 8) | \
   ((  (x) & 0x0000ff00u) <<  8) | ( ( (x) & 0x000000ffu) << 24))

#define __FFDB_BSWAP_64(x) \
  ...
  unsigned long  ntoh_long  (unsigned long lval)
  {
    if (sizeof(long) == sizeof(int))
      return __FFDB_BSWAP_32(lval);
    else
      return __FFDB_BSWAP_64(lval);
  }
